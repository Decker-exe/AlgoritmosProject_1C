...
string getTokenAt(string s, char sep, int i){
    ...
    string sI,sM,sD;
    int t=charCount(s,sep);

    sI=substring(s,0, indexOf(s, sep));

    sM=substring(s,indexOfN(s,sep,i)+1,indexOfN(s,sep,i+1));

    sD=substring(s,indexOfN(s,sep,i)+1);

    return i==0? sI: i==t? sD: sM;
}
...
void removeTokenAt(string& s, char sep, int i){
    ...
    s=(i==0?substring(s,indexOf(s, sep)+1):i==charCount(s,sep)?substring(s,0,indexOfN(s,sep,i)):substring(s,0,indexOfN(s,sep,i))+substring(s,indexOfN(s,sep,i+1)));
}
