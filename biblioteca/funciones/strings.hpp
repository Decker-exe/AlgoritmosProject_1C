...
string intToString(int num) {
    string s = "";
    for (int i = 0; i < digitCount(num); i++) { s = intToChar(getDigit(num, i + 1)) + s; }
    return s;
}
...
bool isEmpty(string s) { return length(s) == 0 ? true : false; }

//22)Determina si la cadena s contiene el caracter c.
bool contains(string s, char c) { return charCount(s, c) > 0 ? true : false; }
...
string removeAt(string s, int pos) {
    for (int i = pos; s[i - 1] != '\0'; i++) { s[i] = s[i + 1]; }
    return s;
...
string ltrim(string s) {
    while (s[0] == ' ') { s = removeAt(s, 0); }
    return s;
}

//27)Recorta los espacios en blanco a la derecha de s.
string rtrim(string s) {
    while (s[length(s) - 1] == ' ') { s = removeAt(s, length(s) - 1); }
    return s;
}
...
string replicate(int n, char c) {
    string s = "";
    for (int pos = 0; pos < n; pos++) { s = insertAt(s, pos, c); }
    return s;
}
...
string cpad(string s, int n, char c) {
    for (int i = 1; length(s) < n; i++) {
        if (i % 2 == 0) { s = rpad(s, length(s) + 1, c); }
        else { s = lpad(s, length(s) + 1, c); }
    }
    return s;
}

//34) Determina si el valor de c corresponde o no a un digito numérico.
bool isDigit(char c) { return c >= 48 and c <= 57 ? true : false; }
...
bool isUpperCase(char c) { return c >= 65 and c <= 90 ? true : false; }

//37) Determina si el valor de c corresponde a una letra minuscula.
bool isLowerCase(char c) { return c >= 97 and c <= 122 ? true : false; }

//38) Convertir el valor de c a mayuscula.
char toUpperCase(char c) { return isLowerCase(c) ? c = c - 32 : c; }

//39) Convertir el valor de c a minuscula.
char toLowerCase(char c) { return isUpperCase(c) ? c = c + 32 : c; }
